...
useEffect(() => {
    ...
    try {
        const respuestas = await fetch("/archivosCSV/03 modern-renewable-prod.csv");
...
const columnas = useMemo(
    ...
    { Header: "ENERGIA EOLICA (TWh)", accessor: "Electricity from wind (TWh)" },
    { Header: "ENERGIA HIDRAULICA (TWh)", accessor: "Electricity from hydro (TWh)" },
    { Header: "ENERGIA SOLAR (TWh)", accessor: "Electricity from solar (TWh)" },
    { Header: "BIOENERGIA (TWh)", accessor: "Other renewables including bioenergy (TWh)" },
    ],
    []
);
...
return (
    <article>
    <div id="segundo-selector" className="filtros-container2">
...
export default TabularProduccionCSV;
